...
function percent(int: number, option1?: any, option2?: any) {
...
const textStory: HTMLElement = document.getElementById("text") as HTMLElement;
const responseButtonElement: HTMLElement = document.getElementById(
  "option-buttons"
) as HTMLElement;
const wText: HTMLElement = document.getElementById("wText") as HTMLElement;
const storyContainer: HTMLElement = document.getElementById(
  "textContainer"
) as HTMLElement;
...
const restart: HTMLElement = document.getElementById("close") as HTMLElement;
...
function writeText(str: string, num: number) {
...
function showTextNode(textNodeIndex: number) {
  ...
  textNode?.options!.forEach((option) => {
    ...
    const buttonStyle =
      "btn text-left border-l-[0.3vh] border-t-[0.3vh] border-b-[0.3vh] border-b-gray-900 border-r-[0.3vh] border-r-gray-900 bg-gray-400 hover:bg-gray-300 my-[1%] py-2 px-4 w-fit h-[7vh] text-[2.2vh] text-gray-800 font-bold overflow-y-auto";
    ...
    if (option.nextText == null) {
      ...
      if (relation >= option.relation.meter) {
        ...
        localStorage.setItem(
          "id",
          convertIdToString(option.relation.nextText1)
        );
      } else {
        writeText(option.text, option.relation.nextText2);
        localStorage.setItem(
          "id",
          convertIdToString(option.relation.nextText2)
        );
      }
    } else {
      text +=option.text + " <hr> ";
...
function convertIdToString(number: number) {
